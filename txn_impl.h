...
transaction<Protocol, Traits>::transaction(uint64_t flags,
...
template<template<typename > class Protocol, typename Traits>
void transaction<Protocol, Traits>::clear() {
...
template<template<typename > class Protocol, typename Traits>
void transaction<Protocol, Traits>::abort_impl(abort_reason reason) {
    abort_trap(reason);
    switch (state) {
        case TXN_EMBRYO:
        case TXN_VALIDATED:
...
template<template<typename > class Protocol, typename Traits>
void transaction<Protocol, Traits>::cleanup_inserted_tuple_marker(
...
namespace {
    ...
    transaction_state_to_cstr(transaction_base::txn_state state) {
        switch (state) {
            case transaction_base::TXN_EMBRYO:
                return "TXN_EMBRYO";
            case transaction_base::TXN_ACTIVE:
                return "TXN_ACTIVE";
            case transaction_base::TXN_ABRT:
                return "TXN_ABRT";
            case transaction_base::TXN_COMMITED:
                return "TXN_COMMITED";
            case transaction_base::TXN_VALIDATED:
...
template<template<typename > class Protocol, typename Traits>
void transaction<Protocol, Traits>::dump_debug_info() const {
...
template<template<typename > class Protocol, typename Traits>
std::map<std::string, uint64_t> transaction<Protocol, Traits>::get_txn_counters() const {
...
template<template<typename > class Protocol, typename Traits>
bool transaction<Protocol, Traits>::handle_last_tuple_in_group(
        dbtuple_write_info &last, bool did_group_insert) {
    if (did_group_insert) {
        // don't need to lock
        if (!last.is_insert())
...
template<template<typename > class Protocol, typename Traits>
bool transaction<Protocol, Traits>::commit(bool doThrow) {
#ifdef TUPLE_MAGIC
    try {
#endif
    ...
    if (!write_set.empty()) {
        ...
        ANON_REGION(probe1_name.c_str(),
                &transaction_base::g_txn_commit_probe1_cg);
        INVARIANT(!is_snapshot());
        typename write_set_map::iterator it = write_set.begin();
        typename write_set_map::iterator it_end = write_set.end();
        for (size_t pos = 0; it != it_end; ++it, ++pos) {
            INVARIANT(!it->is_insert() || it->get_tuple()->is_locked());
            write_dbtuples.emplace_back(it->get_tuple(), &(*it),
                    it->is_insert(), pos);
        }
    }
    ...
    if (!is_snapshot()) {
        ...
        if (!write_dbtuples.empty()) {
            PERF_DECL(
                    static std::string probe2_name(
                            std::string(__PRETTY_FUNCTION__) + std::string(":lock_write_nodes:")));
            ANON_REGION(probe2_name.c_str(),
                    &transaction_base::g_txn_commit_probe2_cg);
            // lock the logical nodes in sort order
            {
                ...
                ANON_REGION(probe6_name.c_str(),
                        &transaction_base::g_txn_commit_probe6_cg);
                write_dbtuples.sort(); // in-place
            }
            typename dbtuple_write_info_vec::iterator it =
                    write_dbtuples.begin();
            typename dbtuple_write_info_vec::iterator it_end =
                    write_dbtuples.end();
            ...
            ANON_REGION(probe5_name.c_str(),
                    &transaction_base::g_txn_commit_probe5_cg);
            commit_tid.second = cast()->gen_commit_tid(write_dbtuples);
            VERBOSE(
                    std::cerr << "commit tid: " << g_proto_version_str(commit_tid.second) << std::endl);
        } else {
        ...
        // do read validation
        {
            ...
            ANON_REGION(probe3_name.c_str(),
                    &transaction_base::g_txn_commit_probe3_cg);
            ...
            if (!read_set.empty()) {
                ...
                for (; it != it_end; ++it) {
                    ...
                    VERBOSE(
                            std::cerr << "validating dbtuple " << util::hexify(it->get_tuple()) << " at snapshot_tid " << g_proto_version_str(cast()->snapshot_tid()) << " FAILED" << std::endl << "  txn read version: " << g_proto_version_str(it->get_tid()) << std::endl << "  tuple=" << *it->get_tuple() << std::endl);
            ...
            if (!absent_set.empty()) {
                ...
                for (; it != it_end; ++it) {
                    const uint64_t v = concurrent_btree::ExtractVersionNumber(
                            it->first);
                    if (unlikely(v != it->second.version)) {
                        VERBOSE(
                                std::cerr << "expected node " << util::hexify(it->first) << " at v=" << it->second.version << ", got v=" << v << std::endl);
                        abort_trap((reason =
        ...
        if (!write_dbtuples.empty()) {
            ...
            ANON_REGION(probe4_name.c_str(),
                    &transaction_base::g_txn_commit_probe4_cg);
            typename write_set_map::iterator it = write_set.begin();
            typename write_set_map::iterator it_end = write_set.end();
            for (; it != it_end; ++it) {
                ...
                INVARIANT(tuple->is_locked());
                VERBOSE(
                        std::cerr << "writing dbtuple " << util::hexify(tuple) << " at commit_tid " << g_proto_version_str(commit_tid.second) << std::endl);
                if (it->is_insert()) {
                    ...
                    const dbtuple::write_record_ret ret =
                            tuple->write_record_at(cast(), commit_tid.second,
                    ...
                    if (unlikely(ret.head_ != tuple)) {
                        ...
                        if (it->get_btree()->insert(varkey(it->get_key()),
                                (typename concurrent_btree::value_type) ret.head_,
                                &old_v, NULL))
                            // should already exist in tree
                            INVARIANT(false);
                        INVARIANT(
                                old_v == (typename concurrent_btree::value_type) tuple);
                    ...
                    if (!it->get_value())
                        ...
                        cast()->on_logical_delete(ret.head_, it->get_key(),
                                it->get_btree());
                    if (unlikely(unlock_head))
                        ret.head_->unlock();
                }
                VERBOSE(
                        std::cerr << "dbtuple " << util::hexify(tuple) << " is_locked? " << tuple->is_locked() << std::endl);
            }
            // unlock
            // NB: we can no longer un-lock after doing the writes above
            for (typename dbtuple_write_info_vec::iterator it =
                    write_dbtuples.begin(); it != write_dbtuples.end(); ++it) {
                if (it->is_locked())
                    it->tuple->unlock();
                else
    ...
    if (this->is_snapshot())
        VERBOSE(
                std::cerr << "aborting txn @ snapshot_tid " << cast()->snapshot_tid() << std::endl);
    else
...
template<template<typename > class Protocol, typename Traits>
void transaction<Protocol, Traits>::write() {
#ifdef TUPLE_MAGIC
    try {
#endif
    ...
    switch (state) {
        case TXN_EMBRYO:
        case TXN_ACTIVE:
            throw std::invalid_argument("Unsupported write without validate"); //Can call validate from here
        case TXN_VALIDATED:
            break;
        case TXN_COMMITED:
    ...
    INVARIANT(commit_tid.first);

    if (!is_snapshot()) {
        // commit actual records
        if (!validated_write_dbtuples.empty()) {
            PERF_DECL(
                    static std::string probe4_name(
                            std::string(__PRETTY_FUNCTION__) + std::string(":write_records:")));
            ANON_REGION(probe4_name.c_str(),
...
template<template<typename > class Protocol, typename Traits>
size_t transaction<Protocol, Traits>::validate() {
#ifdef TUPLE_MAGIC
    try {
#endif
    ...
    if (!write_set.empty()) {
        ...
        for (size_t pos = 0; it != it_end; ++it, ++pos) {
            INVARIANT(!it->is_insert() || it->get_tuple()->is_locked());
            validated_write_dbtuples.emplace_back(it->get_tuple(), &(*it),
                    it->is_insert(), pos);
        }
    }
    ...
    if (!is_snapshot()) {
        ...
        if (!validated_write_dbtuples.empty()) {
            ...
            // lock the logical nodes in sort order
            {
                ...
                ANON_REGION(probe6_name.c_str(),
                        &transaction_base::g_txn_commit_probe6_cg);
                validated_write_dbtuples.sort(); // in-place
            }
            typename dbtuple_write_info_vec::iterator it =
                    validated_write_dbtuples.begin();
            typename dbtuple_write_info_vec::iterator it_end =
                    validated_write_dbtuples.end();
            dbtuple_write_info *last_px = nullptr;
            bool inserted_last_run = false;
            for (; it != it_end; last_px = &(*it), ++it) {
                if (likely(last_px && last_px->tuple != it->tuple)) {
...
template<template<typename > class Protocol, typename Traits>
std::pair<dbtuple *, bool > transaction<Protocol, Traits>::try_insert_new_tuple(
        concurrent_btree &btr, const std::string *key, const void *value,
        dbtuple::tuple_writer_t writer) {
    INVARIANT(key);
    const size_t sz =
            value ? writer(dbtuple::TUPLE_WRITER_COMPUTE_NEEDED, value, nullptr,
                            0) :
                    0;
    ...
    if (value)
        writer(dbtuple::TUPLE_WRITER_DO_WRITE, value, tuple->get_value_start(),
                0);
    ...
    VERBOSE(
            std::cerr << "insert_if_absent suceeded for key: " << util::hexify(key) << std::endl << "  new dbtuple is " << util::hexify(tuple) << std::endl);
    // update write_set
    ...
    if (!absent_set.empty()) {
        ...
        if (it != absent_set.end()) {
            ...
            VERBOSE(
                    std::cerr << "bump node=" << util::hexify(it->first) << " from v=" << insert_info.old_version << " -> v=" << insert_info.new_version << std::endl);
            // otherwise, bump the version
            it->second.version = insert_info.new_version;
            SINGLE_THREADED_INVARIANT(
                    concurrent_btree::ExtractVersionNumber(it->first) == it->second);
        }
    }
...
bool transaction<Protocol, Traits>::do_tuple_read(const dbtuple *tuple,
        ValueReader &value_reader) {
    INVARIANT(tuple);
    ++evt_local_search_lookups;

    const bool is_snapshot_txn = is_snapshot();
    const transaction_base::tid_t snapshot_tid =
            is_snapshot_txn ?
                    cast()->snapshot_tid() :
                    static_cast<transaction_base::tid_t>(dbtuple::MAX_TID);
    ...
    if (Traits::read_own_writes) {
        ...
        if (unlikely(write_set_it != write_set.end())) {
            ...
            const typename ValueReader::value_type * const px =
                    reinterpret_cast<const typename ValueReader::value_type *>(write_set_it->get_value());
    ...
    dbtuple::ReadStatus stat;
    {
        ...
        tuple->prefetch();
        stat = tuple->stable_read(snapshot_tid, start_t, value_reader,
                this->string_allocator(), is_snapshot_txn);
...
template<template<typename > class Protocol, typename Traits>
void transaction<Protocol, Traits>::do_node_read(
